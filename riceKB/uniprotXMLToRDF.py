...
#        file_handle = open(upfile, "r")
    with open(up_files, 'r') as file_handle:
        ...
        #        xref_ids = list()
        for record in SeqIO.UniprotIO.UniprotIterator(file_handle):#SwissProt.parse(file_handle): #up_records:
            xrefs = defaultdict(list)
            print(record)
            print(dir(record))
            ...
            for key, value in record.annotations.items():
                print(">>>\t" + key )
                print(record.annotations[key])
                if "Reference" in key:
                    for key1, value1 in record.annotations[key]:
                        print(key1)
            #ref_record = SwissProt._read_rx(record.references,'RX')
            rdf_buffer = ''
            for taxID in record.__getattribute__():
